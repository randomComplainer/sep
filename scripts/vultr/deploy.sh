...
EOF
	...
	ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${ip} >&2 <<EOF
		...
		sed -i 's/{cert}/\/root\/test_assets\/server.cert.pem/g' /etc/systemd/system/sep-server.service;
		sed -i 's/{key}/\/root\/test_assets\/server.key.pem/g' /etc/systemd/system/sep-server.service;
		sed -i 's/{trusted_dir}/\/root\/test_assets\/trusted/g' /etc/systemd/system/sep-server.service;
