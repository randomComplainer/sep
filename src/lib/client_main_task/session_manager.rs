    ...
    ProxyeeStream: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    // TODO: magic number
    let (local_session_ending_tx, mut local_session_ending_rx) = futures::channel::mpsc::channel::<u16>(4);
    ...
    loop {
        tokio::select! {
            ...
            // be sure to break the loop so the scope task does not get polled again
            // (it should never end anyway )
            _ = &mut sessions_scope_task => {
                panic!("session scope task ended unexpectedly");
            }
        }
    }
}
